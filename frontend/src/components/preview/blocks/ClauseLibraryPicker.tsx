...
// ---------------------------------------------------------------------------
// Section slug detection from heading text
// ---------------------------------------------------------------------------
const HEADING_TO_SECTION: [RegExp, string][] = [
...
// Component
// ---------------------------------------------------------------------------
export interface ClauseLibraryPickerProps {
  /** Commodity key e.g. "APPLE", "MANDARIN" */
  commodity?: string;
  /** Raw heading text from the block, used to auto-detect section */
  sectionHeading?: string;
  /** Called when the surveyor clicks a clause to insert */
  onInsert: (text: string) => void;
}

export const ClauseLibraryPicker: React.FC<ClauseLibraryPickerProps> = ({
  commodity,
  sectionHeading = '',
  onInsert,
}) => {
  ...
  // Load clauses lazily when panel opens
  const loadClauses = useCallback(async () => {
    const key = `${commodity}__${section}`;
    if (fetchedKey.current === key) return; // already loaded for this context
    setLoading(true);
    setError(null);
    try {
  ...
  // Reset inserted state when commodity/section changes
  useEffect(() => {
    setInserted(new Set());
    setOpen(false);
    fetchedKey.current = '';
